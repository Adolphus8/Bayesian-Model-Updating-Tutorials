function [output] = TMCMCsampler2(varargin)
...
while pj < 1    
    ...
    pj1 = calculate_pj1(nsamples, log_fD_T_thetaj, pj);
...
%% Calculate the tempering parameter p(j+1)
function pj1 = calculate_pj1(nsamples, log_fD_T_thetaj, pj)
% find pj1 such that the Effective Sample Size (ESS) equals N/2:
%
%         1
% ------------------ >= threshold
%  sum[(hat_wj)^2]
...
fmin = @(e) (sum(wj(e)))^2 - (threshold .* sum((wj(e)).^2)) + realmin; 
