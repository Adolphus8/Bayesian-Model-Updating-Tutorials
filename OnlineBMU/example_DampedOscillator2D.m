...
%
% The spring stiffness and damper are time-varying and weakens over time such
% that:
%
% k(t) = 1.5*(1 + exp(-alpha*t)); where alpha = 0.03 s^-1 (rate parameter);
% c(t) = 0.55*exp(-beta*t); where beta = 0.4 s^-1 (rate parameter);
%
% Discretizing the above equations, we get the following discretized
% dynamic equation:
%
% k(t+1) = exp(-alpha*t).*(k(t) - 1.5) + 1.5;
% c(t+1) = exp(-beta*t).*c(t);
%
...
% Define model for the damper, c:
beta = 0.40;     % Rate parameter [s^-1]
c = @(t) 0.55.*exp(- beta .* t);

% Define model for the eigenfrequency, omega:
omega = @(t,m) sqrt((k(t)./m) - (c(t)./(2.*m)).^2);

% Define model for simple harmonic oscillator:
delta = 0; % Initial phase term when time starts [rad] 
displacement = @(t_d,t_i) A.*exp(- (c(t_i)./(2.*m)).*t_d).*cos((omega(t_i,m) .* t_d) + delta); % in [m]
...
subplot(1,2,2)
hold on; grid on; box on;
plot(t_i, c(t_i), 'k--', 'linewidth', 1)
xlabel('Inspection time, t_i [mth]')
ylabel('Damping coefficient, c(t_i) [N.s/m]')
xlim([0 12])
set(gca, 'fontsize', 20)
...
% Define the dynamical model for k(t+1)|k(t) and c(t+1)|c(t):
k_new = @(k_old) exp(- alpha).*(k_old - 1.5) + 1.5;
c_new = @(c_old) exp(- beta).*c_old;
dynamic_model = @(x) [k_new(x(:,1)), c_new(x(:,2))];

% Define the inverse model for k(t)|k(t+1) and c(t)|c(t+1):
k_old = @(k_new) exp(alpha).*(k_new - 1.5) + 1.5;
c_old = @(c_new) exp(beta).*c_new;
inverse_model = @(x) [k_old(x(:,1)), c_old(x(:,2))];
...
posterior_bounds_c = zeros(length(t_i),2);
...
prediction_bounds_c = zeros(length(t_i),2);
...
prediction_mean(idx,2) = mean(semc_prediction(:,2,idx));
...
prediction_std(idx,2) = std(semc_prediction(:,2,idx));

posterior_bounds_k(idx,:) = prctile(semc_allsamples(:,1,idx+1), [5, 95]);
posterior_bounds_c(idx,:) = prctile(semc_allsamples(:,2,idx+1), [5, 95]);

prediction_bounds_k(idx,:) = prctile(semc_prediction(:,1,idx), [5, 95]);
prediction_bounds_c(idx,:) = prctile(semc_prediction(:,2,idx), [5, 95]);
end
...
hold on; box on;
for idx = 1:length(t_i)
histogram(semc_allsamples(:,2,idx+1),10)
end
xlim([0 0.65])
xlabel('Damping coefficient, c(t_i) [N/m]')
ylabel('Counts')
legend('t_i = 0 mth', 't_i = 1 mth', 't_i = 2 mth', 't_i = 3 mth', 't_i = 4 mth',...
       't_i = 5 mth', 't_i = 6 mth', 't_i = 7 mth', 't_i = 8 mth', 't_i = 9 mth',...
       't_i = 10 mth', 't_i = 11 mth', 't_i = 12 mth', 'LineWidth', 2)
...
% To plot the estimates of k(t_i) and c(t_i) wih the theoretical model:
figure;
subplot(1,2,1)
hold on; grid on; box on;
plot(t_i, k(t_i), 'k--', 'linewidth', 1)
...
hold on; grid on; box on;
plot(t_i, c(t_i), 'k--', 'linewidth', 1)
y_neg_b = abs(posterior_mean(:,2) - posterior_bounds_c(:,1)); % error in the negative y-direction
y_pos_b = abs(posterior_mean(:,2) - posterior_bounds_c(:,2)); % error in the positive y-direction
errorbar(t_i, posterior_mean(:,2), y_neg_b, y_pos_b, '-s','MarkerSize',5,...
    'MarkerEdgeColor','blue','MarkerFaceColor','blue', 'linewidth',1);
y_neg_b2 = abs(prediction_mean(:,2) - prediction_bounds_c(:,1)); % error in the negative y-direction
y_pos_b2 = abs(prediction_mean(:,2) - prediction_bounds_c(:,2)); % error in the positive y-direction
errorbar((t_i(2:end)), prediction_mean(1:end-1,2), y_neg_b2(1:end-1), y_pos_b2(1:end-1), '-s','MarkerSize',5,...
    'MarkerEdgeColor','red','MarkerFaceColor','red', 'linewidth',1);
legend('True values', 'Estimated values', 'Predicted values', 'linewidth', 2)
xlim([0 12])
xlabel('Inspection time, t_i [mth]')
ylabel('Damping coefficient, c(t_i) [N/m]')
set(gca, 'fontsize', 18)
...
save('example_DampedOscillator2D');
